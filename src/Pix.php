...
class Pix
{
    ...
    {
        return $this->client->post(self::PIX_ENDPOINT, $data);
    }

    public function confirmPayment(string $pixPaymentId, array $data): array
    {
        if ($pixPaymentId === '') {
            throw new \InvalidArgumentException('pix_payment_id not provided');
        }

        return $this->client->patch(self::PIX_ENDPOINT . '/' . $pixPaymentId, $data);
    }

    public function listPayments(array $params = []): array
    {
        return $this->client->get(self::PIX_ENDPOINT, $params === [] ? null : $params);
    }

    public function getPayment(string $pixPaymentId): array
    {
        if ($pixPaymentId === '') {
            throw new \InvalidArgumentException('pix_payment_id not provided');
        }
    ...
    {
        return $this->getPayment($pixPaymentId);
    }
